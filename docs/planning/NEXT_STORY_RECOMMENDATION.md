...
GUP-032 just completed the Mark system. Examples use marks extensively. Fixing them now:
...
The ShaderFunction API updates that broke examples are related to the work we just did:
...
GUP-135 completes the validation loop:
✅ GUP-032: Built advanced marks
→ GUP-135: Prove they work in examples
→ Ready for next Phase 1 initiative

## Recommended Sequence

1. **Now**: GUP-135 (Fix Examples) - 3 points
2. **Next**: GUP-131 (Shader Constructors) - 1 point
3. **Then**: GUP-031 (GPU Interaction) or GUP-033 (Shader Composition) - Major features
