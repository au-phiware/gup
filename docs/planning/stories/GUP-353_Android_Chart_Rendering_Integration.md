...
touch translation) but the `gup_render_frame()` function is a stub that returns
`true` without producing visible output. This story wires the chart builder
pipeline into the Android render path so that `GupSurfaceView` displays actual
GPU-accelerated data visualisations.

This mirrors GUP-272 (iOS Chart Rendering Integration) which does the same for
the iOS platform.

## User Story

> "As an Android app developer, I want to see actual chart content rendered in
> my `GupSurfaceView` so that I can use Gup for real data visualisation rather
> than a blank surface."

## Acceptance Criteria

- [ ] `gup_render_frame()` acquires the current surface texture, renders a chart
      scene, and presents the frame.
- [ ] A Kotlin-side API allows setting the chart configuration (data source,
      mark type, scales) on the `GupSurfaceView`.
- [ ] The example app in `examples/android/` displays a visible line chart with
      simulated streaming data.
- [ ] Frame timing remains under 16ms (60 fps) on the CI emulator with ≤ 1,000
      data points.
