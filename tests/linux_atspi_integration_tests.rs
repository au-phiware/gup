...
mod linux_atspi_tests {
    use gup::accessibility::{
        AccessibilitySystem, AnnouncementPriority, AriaNode, AriaRole,
    };
    ...
    fn test_atspi_announcement() {
        ...
        let result = accessibility.announce(
            "Test chart data loaded",
    ...
    fn test_atk_role_mapping() {
        ...
        assert_eq!(AtkRole::from_aria_role(&AriaRole::ChartSeries), AtkRole::Panel);
        assert_eq!(AtkRole::from_aria_role(&AriaRole::DataPoint), AtkRole::Label);
        ...
        assert_eq!(AtkRole::from_aria_role(&AriaRole::Tooltip), AtkRole::ToolTip);
