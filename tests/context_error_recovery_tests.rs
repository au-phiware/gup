...
async fn test_recovery_with_fallback_options() {
    ...
    let result = context_mut
        .attempt_recovery()
        .await
        .expect("Failed to get recovery result");
...
async fn test_recovery_attempt() {
    ...
    let last_attempt = context_mut
        .last_recovery_attempt()
        .expect("No recovery attempt recorded");
