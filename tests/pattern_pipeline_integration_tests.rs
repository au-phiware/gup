...
async fn test_pattern_type_uniforms() -> GupResult<()> {
    ...
    for (pattern, expected_type_id) in patterns {
        let uniforms = PatternUniforms::from_pattern(&pattern, Color::BLACK, Color::WHITE);
        assert_eq!(
            uniforms.pattern_type, expected_type_id,
            "Pattern type ID mismatch for {:?}",
            pattern
        );
    }
...
async fn test_pattern_parameters() -> GupResult<()> {
    ...
    let crosshatch_uniforms =
        PatternUniforms::from_pattern(&crosshatch, Color::BLACK, Color::WHITE);
...
async fn test_pattern_rendering_performance() -> GupResult<()> {
    ...
    if standard_duration.as_nanos() > 0 {
        ...
        assert!(
            overhead_ratio < 3.0,
            "Pattern pipeline creation overhead too high: {:.2}x",
            overhead_ratio
        );
    }
    ...
    assert!(
        standard_duration.as_millis() < 100,
        "Standard pipeline creation too slow"
    );
    assert!(
        pattern_duration.as_millis() < 100,
        "Pattern pipeline creation too slow"
    );
