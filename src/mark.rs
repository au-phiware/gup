...
mod tests {
    ...
    // ============================================================
    // Mark derive macro tests
    // ============================================================

    use crate::shader_function::Vec2;

    /// Test: basic quad mark generated by derive macro
    #[derive(Debug, Clone, gup_macros::Mark)]
    #[mark(primitive = "quad")]
    #[allow(dead_code)]
    pub struct DerivedQuadMark {
        pub center: Vec2,
        pub size: f32,
        pub color: Vec4,
    }
    ...
    #[test]
    fn test_derived_quad_mark_vertex_count() {
        assert_eq!(DerivedQuadMark::vertex_count(), 4);
    }

    #[test]
    fn test_derived_quad_mark_index_count() {
        assert_eq!(DerivedQuadMark::index_count(), Some(6));
    }

    #[test]
    fn test_derived_quad_mark_generate_vertices() {
        let vertices = DerivedQuadMark::generate_vertices();
        assert_eq!(vertices.len(), 4);
        assert_eq!(vertices[0].position, [-1.0, -1.0]);
    ...
    #[test]
    fn test_derived_quad_mark_generate_indices() {
        let indices = DerivedQuadMark::generate_indices().unwrap();
        assert_eq!(indices, vec![0, 1, 2, 0, 2, 3]);
    }

    #[test]
    fn test_derived_quad_mark_attribute_types() {
        assert_eq!(
            DerivedQuadMark::get_attribute_type("center").unwrap(),
            "vec2<f32>"
        );
        assert_eq!(DerivedQuadMark::get_attribute_type("size").unwrap(), "f32");
        assert_eq!(
            DerivedQuadMark::get_attribute_type("color").unwrap(),
            "vec4<f32>"
        );
    }

    #[test]
    fn test_derived_quad_mark_unknown_attribute() {
        assert!(DerivedQuadMark::get_attribute_type("nonexistent").is_err());
    }

    #[test]
    fn test_derived_quad_mark_vertex_bytemuck() {
        let vertex = DerivedQuadMarkVertex {
            position: [1.0, 2.0],
        };
        let bytes = bytemuck::bytes_of(&vertex);
        assert_eq!(bytes.len(), std::mem::size_of::<DerivedQuadMarkVertex>());
        assert_eq!(bytes.len(), 8); // 2 * f32
    }

    /// Test: triangle mark generated by derive macro
    #[derive(Debug, Clone, gup_macros::Mark)]
    #[mark(primitive = "triangle")]
    #[allow(dead_code)]
    pub struct DerivedTriangleMark {
        pub center: Vec2,
        pub color: Vec4,
    }

    #[test]
    fn test_derived_triangle_mark_vertex_count() {
        assert_eq!(DerivedTriangleMark::vertex_count(), 3);
    }

    #[test]
    fn test_derived_triangle_mark_index_count() {
        assert_eq!(DerivedTriangleMark::index_count(), None);
    }

    #[test]
    fn test_derived_triangle_mark_generate_vertices() {
        let vertices = DerivedTriangleMark::generate_vertices();
        assert_eq!(vertices.len(), 3);
    ...
    #[test]
    fn test_derived_triangle_mark_generate_indices() {
        assert!(DerivedTriangleMark::generate_indices().is_none());
    }

    /// Test: mark with default primitive (should be quad)
    #[derive(Debug, Clone, gup_macros::Mark)]
    #[allow(dead_code)]
    pub struct DefaultPrimitiveMark {
        pub position: Vec2,
        pub radius: f32,
    }

    #[test]
    fn test_default_primitive_is_quad() {
        assert_eq!(DefaultPrimitiveMark::vertex_count(), 4);
        assert_eq!(DefaultPrimitiveMark::index_count(), Some(6));
    }

    /// Test: mark attribute compatibility
    #[test]
    fn test_derived_mark_attribute_compatibility() {
        assert!(DerivedQuadMark::is_attribute_compatible(
            "center",
            "vec2<f32>"
        ));
        assert!(DerivedQuadMark::is_attribute_compatible(
        ...
        assert!(!DerivedQuadMark::is_attribute_compatible("center", "f32"));
    }

    /// Test: derived mark has no custom shaders by default
    #[test]
    fn test_derived_mark_no_custom_shaders() {
        assert!(DerivedQuadMark::VERTEX_SHADER.is_none());
        assert!(DerivedQuadMark::FRAGMENT_SHADER.is_none());
        assert!(DerivedQuadMark::PATTERN_FRAGMENT_SHADER.is_none());
    }
}
